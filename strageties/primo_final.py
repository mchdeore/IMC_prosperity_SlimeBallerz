...
HARDCODED PARAMETERS (all from results/primo_exploration/ sweeps):
    ...
    INTARIAN_PEPPER_ROOT (IPR) - SAFE mode (fallback market maker):
        ...
        Uses book-derived mid as fair (no slope assumption), symmetric
        50/50 volume split, always flattens toward 0, and posts BACKED-OFF
...
class Trader:
    ...
    def run_ipr_linear(self, depth, position, timestamp, ipr_state):
        ...
        if "initial_fair" not in ipr_state:
            ...
            best_bid = max(depth.buy_orders)
            best_ask = min(depth.sell_orders)
            ipr_state["initial_fair"] = (best_bid + best_ask) / 2.0
        ...
        # Long-bias pressure: only fight SHORT positions past softcap (let
        # long positions ride - drift will bail us out).
        if position < -IPR_SOFT_CAP:
            abs_pos = abs(position)
            excess = (abs_pos - IPR_SOFT_CAP) / float(POSITION_LIMIT - IPR_SOFT_CAP)
            if excess > 1.0:
                excess = 1.0
            shift = int(round(excess * (fair - bid_price)))
            new_bid = bid_price + shift
    ...
    # its peak). Drops all drift assumptions. Uses book-derived mid as
    # fair, symmetric everything, 50% make volume, always flattens toward
