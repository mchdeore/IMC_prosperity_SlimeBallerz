"""
Spy: ASH_COATED_OSMIUM Experiment Runner
=========================================
Systematically probes bot behavior for ACO by cycling through 4 experiment
types on a 30-tick rotation:

  1. Edge sweep   (12 ticks) — quote offsets 1-15 from wall_mid
  2. Volume sweep  (7 ticks) — vary lot size at fixed edge=5
  3. Take strategy  (4 ticks) — passive / aggressive / overbid / wallmatch
  4. Skew test      (7 ticks) — vary bid/ask volume ratio

Each tick places one small experiment, checks whether the previous tick's
probe filled, and accumulates fill-rate counters in traderData.

Submit to the Prosperity platform. Parse logs with log_parser.py.
Only trades ASH_COATED_OSMIUM; ignores other products.
"""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if prev and PRODUCT in state.own_trades:
            ...
            prev_result = {"key": key, "filled": filled, "fill_qty": fill_qty}
        saved["cum"] = cum
        ...
        # ── Run experiment ────────────────────────────────────────────
            ...
            if exp_type == "edge":
                ...
                if mode == "passive":
                    ...
                    if o:
                        orders.append(o)
                        probe = {"exp": "take", "param": mode, "side": side, "px": px, "qty": abs(qty)}
        ...
        anchor_dist = round(wm - ANCHOR, 1) if wm is not None else None
...
if __name__ == "__main__":
    ...
    t = Trader()
    print("=" * 60)
    print("SPY_ACO SMOKE TEST")
    print("=" * 60)

    td = ""
    for i in range(35):
        mid_base = 10000 + (i % 3) - 1
        s = make_state(
            {mid_base - 2: 10, mid_base - 4: 20, mid_base - 6: 15},
            {mid_base + 2: -10, mid_base + 4: -20, mid_base + 6: -15},
    ...
    s = make_state(
        {9998: 10, 9996: 20}, {10002: -10, 10004: -20},
